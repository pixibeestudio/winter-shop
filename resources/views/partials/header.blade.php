<header class="fixed w-full top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 transition-all duration-300">
    <div class="container mx-auto px-6 h-20 flex justify-between items-center">
        <a href="/" class="flex items-center gap-2 group">
            <div class="w-8 h-8 rounded-full bg-brand-dark group-hover:bg-brand transition-colors"></div>
            <span class="font-bold text-xl tracking-wide text-brand-dark">WINTER<span class="font-light">STORE</span></span>
        </a>
        ...
        <div class="flex items-center gap-4">
            ...
            <button onclick="toggleCart(true)" class="p-2 hover:bg-gray-100 rounded-full transition-colors relative">
                ...
                <span class="absolute top-0 right-0 w-4 h-4 bg-red-500 text-white text-[10px] font-bold flex items-center justify-center rounded-full">0</span>
            </button>

            <button class="hidden md:block bg-brand-dark text-white px-5 py-2 rounded-full text-sm font-semibold hover:bg-opacity-90 hover:shadow-lg transition-all">
                Login
            </button>
        </div>
    </div>
